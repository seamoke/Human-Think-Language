...
def show(mask,dtype):
    ...
    plt.imshow(mask.cpu().int().numpy(), cmap='viridis')
    plt.colorbar()  # 显示颜色条，可选
    plt.savefig('./my_tensor_image.png')
    pdb.set_trace()
    ...
    def _make_causal_mask(
        ...
        if segment is not None:
            for i,seg in enumerate(segment):
                ...
                data_list = np.random.choice(total_number, int(total_number * mask_ratio), replace=False)

                # 计算位置
                pos_x = np.minimum(tgt_len - 1, seg[1] + data_list // row)
                pos_y = data_list % row + 4

                # 使用PyTorch的索引和广播功能进行更新
                mask[i, 0, pos_x, pos_y] = torch.finfo(dtype).min
                #data_list = random.sample(data_list, int(total_number*mask_ratio))
                #pos_list = [(min(tgt_len-1,seg[1]+(x//row)), x%row+4) for x in data_list]
...
def _prepare_4d_causal_attention_mask(
    ...
    if attention_mask is not None and len(attention_mask.shape) == 2:
        attention_mask = attn_mask_converter.to_4d(
            attention_mask, input_shape[-1], key_value_length=key_value_length, dtype=inputs_embeds.dtype,mask_ratio=mask_ratio
        )
        ...
        attention_mask = attn_mask_converter.to_causal_4d(
            input_shape[0], input_shape[-1], key_value_length, dtype=inputs_embeds.dtype, device=inputs_embeds.device, mask_ratio = mask_ratio
        )
...
def _create_4d_causal_attention_mask(
    ...
    attention_mask = attn_mask_converter.to_causal_4d(
        input_shape[0], input_shape[-1], key_value_length, dtype=dtype, device=device,mask_ratio=mask_ratio
    )
