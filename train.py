...
import utils
import random
os.environ["WANDB_PROJECT"] = "mammoth"
IGNORE_INDEX = -100
DEFAULT_PAD_TOKEN = "[PAD]"
DEFAULT_EOS_TOKEN = "</s>"
DEFAULT_BOS_TOKEN = "<s>"
DEFAULT_UNK_TOKEN = "<unk>"
DEFAULT_SPLIT_TOKEN = "<cot>"
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, template_variation: bool,insert_args:float):
        ...
        if data_path=='mine':
            ...
            with open('your_path/math_eval/dataset/mistral/gsm8k.json', 'r') as f:
                tune_list = json.load(f)
                print(f'gsm8k :{len(tune_list)}')
            with open('your_path/math_eval/dataset/mistral/math_tune.json', 'r') as f:
                temp_list = json.load(f)
                tune_list += temp_list
                print(f'math :{len(temp_list)}')
            with open('your_path/math_eval/dataset/mistral/numglue.json', 'r') as f:
                temp_list = json.load(f)
                tune_list += temp_list
            ...
            for x in tune_list:
                ...
                # print(cot,pot)
                # insert_list.append({'question':x['question'],'COT':cot,'POT':pot})
                # if random.random()<insert_args:
                #     cot,pot = x['output'].split('****')
                ...
                cot,pot = x['output'].split('****')
                x['output'] = f"<cot>{cot}<cot>****{pot}"
                sources.append(prompt_all.format(instruction=x['question']))
                targets.append(f"{x['output']}{tokenizer.eos_token}")
            data_list = list(range(len(tune_list)))
            data_list = random.sample(data_list, int(len(tune_list)*insert_args))
            for id in data_list:
                x = tune_list[id]
                cot,pot = x['output'].split('****') 
                sources.append(prompt_insert.format(instruction=x['question'],cot=cot))
        ...
        print("**********")
        print(sources[-1],targets[-1])
        self.sources = sources[0:1000]
        self.targets = targets[0:1000]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    train_dataset = SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path,
                                      ...
                                      insert_args=data_args.insert_args)
...
def train():
   ...
   # model = torch.compile(model)
    ...
    special_tokens_dict['additional_special_tokens'] = [DEFAULT_SPLIT_TOKEN] #32017
    ...
    total_batch = training_args.gradient_accumulation_steps*training_args.per_device_train_batch_size
